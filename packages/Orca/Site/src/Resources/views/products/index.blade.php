...
@section('content-wrapper')
    @inject ('productRepository', 'Orca\Product\Repositories\ProductRepository')

    <div class="main">
        {!! view_render_event('orca.site.products.index.before', ['category' => $category]) !!}

        <div class="category-container">

            @if (in_array($category->display_mode, [null, 'products_only', 'products_and_description']))
                @include ('site::products.list.layered-navigation')
            @endif
            ...
            <div class="category-block" @if ($category->display_mode == 'description_only') style="width: 100%" @endif>
                ...
                @if (in_array($category->display_mode, [null, 'products_only', 'products_and_description']))
                    ...
                    @if ($products->count())
                        ...
                        @include ('site::products.list.toolbar')

                        @inject ('toolbarHelper', 'Orca\Product\Helpers\Toolbar')
                        ...
                        {!! view_render_event('orca.site.products.index.pagination.before', ['category' => $category]) !!}

                        <div class="bottom-toolbar">
                            {{ $products->appends(request()->input())->links() }}
                        </div>

                        {!! view_render_event('orca.site.products.index.pagination.after', ['category' => $category]) !!}
        ...
        {!! view_render_event('orca.site.products.index.after', ['category' => $category]) !!}
    </div>
@stop
